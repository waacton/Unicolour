...
internal static class VisionDeficiency
{
    ...
    // Machado et al. (2009) showed results on gamma-encoded sRGB as it did not appear to make a difference (https://github.com/aalto-ui/aim/pull/13#issuecomment-954062715)
    ...
    internal static Unicolour Simulate(Cvd cvd, double severity, Unicolour colour)
    {
        ...
        {
            Cvd.Protan => ApplyCvdMatrix(colour, Protanomaly, severity),
            Cvd.Deutan => ApplyCvdMatrix(colour, Deuteranomaly, severity),
            Cvd.Tritan => ApplyCvdMatrix(colour, Tritanomaly, severity),
    ...
    private static ColourTriplet ApplyCvdMatrix(Unicolour colour, double[][,] matrices, double severity)
    {
        if (severity is <= 0 or double.NaN) return colour.RgbLinear.Triplet;
        
        var cvdMatrix = severity switch
        {
            >= 1 => matrices.Last(),
            _ => InterpolateMatrices(matrices, severity)
        };
        
        // since simulated RGB-Linear often results in values outwith 0 - 1, seems unnecessary to use constrained inputs
        return new Matrix(cvdMatrix).Multiply(Matrix.From(colour.RgbLinear)).ToTriplet();
    }

    internal static double[,] InterpolateMatrices(double[][,] matrices, double severity)
    {
        const int rows = 3;
        const int cols = 3;
